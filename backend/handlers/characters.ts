...
export async function createCharacter(req: Request, res: Response): Promise<void> {
	...
	if (typeof player !== 'string' || player.trim() === '') {
		res.status(400).json({ error: 'player must be a non-empty string' });
		return;
	}
	if (typeof hp_max !== 'number' || !Number.isFinite(hp_max) || hp_max <= 0) {
		res.status(400).json({ error: 'hp_max must be a positive finite number' });
		return;
	}
	if (
		hp_current !== undefined &&
		(typeof hp_current !== 'number' || !Number.isFinite(hp_current) || hp_current < 0)
	) {
		res
			.status(400)
			.json({ error: 'hp_current must be a finite number greater than or equal to 0' });
		return;
	}
	if (portrait !== undefined && typeof portrait !== 'string') {
		res.status(400).json({ error: 'portrait must be a string' });
		return;
	}
...
export async function updateHp(req: Request, res: Response): Promise<void> {
	...
	if (hp_current === undefined || typeof hp_current !== 'number' || !Number.isFinite(hp_current)) {
		res.status(400).json({ error: 'hp_current must be a finite number' });
		return;
	}
	let character;
	try {
...
export async function updatePhoto(req: Request, res: Response): Promise<void> {
	...
	if (typeof portrait === 'string' && portrait.length > 2000000) {
		res.status(413).json({ error: 'portrait payload is too large' });
		return;
	}
	let character;
	try {
...
export async function updateCharacter(req: Request, res: Response): Promise<void> {
	...
	const num = (v: unknown, field: string, min = 0): boolean => {
		if (typeof v !== 'number' || !Number.isFinite(v) || v < min) {
			res.status(400).json({ error: `${field} must be a number >= ${min}` });
			return false;
		}
		return true;
	};
	...
	if (b.proficiency_bonus !== undefined && !num(b.proficiency_bonus, 'proficiency_bonus', 1))
		return;
...
export async function addCondition(req: Request, res: Response): Promise<void> {
	...
	if (typeof condition_name !== 'string' || condition_name.trim() === '') {
		res.status(400).json({ error: 'condition_name must be a non-empty string' });
		return;
	}
	if (
		...
		res.status(400).json({ error: 'intensity_level must be a positive number' });
		return;
	}
	let character;
	try {
	...
	const condition = character.conditions?.at(-1) ?? null;
...
export async function removeCondition(req: Request, res: Response): Promise<void> {
	...
	if (!SHORT_ID_RE.test(condId)) {
		res.status(400).json({ error: 'condId must be 5 chars' });
		return;
	}
	let ok: boolean;
	try {
		...
		if ((err as any)?.status === 404) {
			res.status(404).json({ error: 'Character or condition not found' });
			return;
		}
		throw err;
	}
...
export async function updateResource(req: Request, res: Response): Promise<void> {
	...
	if (pool_current === undefined || pool_current === null) {
		res.status(400).json({ error: 'pool_current required' });
		return;
	}
	if (typeof pool_current !== 'number' || !Number.isFinite(pool_current)) {
		res.status(400).json({ error: 'pool_current must be a number' });
		return;
	}
	if (pool_current < 0) {
		res.status(400).json({ error: 'pool_current must be >= 0' });
		return;
	}
	let resource;
	try {
		...
		if ((err as any)?.status === 404) {
			res.status(404).json({ error: 'Character or resource not found' });
			return;
		}
		throw err;
	}
...
export async function restoreResources(req: Request, res: Response): Promise<void> {
	...
	if (!['short', 'long'].includes(type)) {
		res.status(400).json({ error: 'type must be "short" or "long"' });
		return;
	}
	let result;
	try {
