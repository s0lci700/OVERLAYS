...
export interface SubclassOption extends OptionItem {
	class?: string; // parent class key; used to filter by selected class
}
...
export function parseOptionSets(characterOptions: Record<string, OptionItem[]>): OptionSets {
...
export function buildCharacterPayload(fields: CharacterFormValues): Partial<CharacterRecord> {
	...
	const proficiency_bonus = level >= 17 ? 6 : level >= 13 ? 5 : level >= 9 ? 4 : level >= 5 ? 3 : 2;
		...
		...(skills.length > 0
			? {
					skill_proficiencies: Object.fromEntries(
						skills.map((s) => [s, true])
					) as import('$lib/contracts/records').SkillProfs
				}
			: {}),
		is_active: true,
