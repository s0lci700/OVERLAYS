		...
		socket.on('character_updated', ({ character }) => {
			...
			charState = {
				...
				[character.id]: {
					...
					conditions: character.conditions
						? [...character.conditions]
						: charState[character.id].conditions,
					resources: character.resources
						? [...character.resources]
						: charState[character.id].resources
				}
			};
			updateVisibility();
		});
...
{#if panelVisible}
	<div bind:this={panelEl} class="conditions-panel" style="opacity: 0">
		...
		{#each visibleRows() as [charId, entry] (charId)}
			...
			<div class="char-condition-row" class:depleted-only={onlyDepleted} data-char-id={charId}>
...
<style>
	...
	.char-condition-row.depleted-only {
		background: rgba(80, 13, 245, 0.06);
		border-color: rgba(80, 13, 245, 0.4);
	}
