...
<script lang="ts">
	...
	let {
		...
		SERVER_URL: string;
		onProfileSaved?: (_id: string, _payload: unknown) => void;
		onLevelUp?: (_id: string) => void;
	} = $props();
	...
	let _prevClassPrimary = classPrimary;
	$effect(() => {
		if (classPrimary !== _prevClassPrimary) {
			_prevClassPrimary = classPrimary;
			classSubclass = '';
		}
	});
	...
	<div class="manage-section">
		...
		<div class="manage-photo-editor-inline">
			<PhotoSourcePicker
				...
				onError={(msg) => {
					if (msg) feedback = { type: 'error', text: msg };
				}}
			/>
		</div>
	</div>
