...
<script>
	...
	/**
	 * @typedef {{key: string, label: string, class?: string}} OptionEntry
	 * @typedef {{
	 *  classes?: OptionEntry[],
	...
	const rareLanguageKeys = new Set(rareLanguageOptions.map((option) => option.key));
	const labelMaps = {
	...
	function initCharacterState(character) {
		...
		const backgroundTools = background.tool_proficiency ? [background.tool_proficiency] : [];
	...
	async function savePhoto(charId) {
		...
		try {
			const response = await fetch(`${SERVER_URL}/api/characters/${charId}/photo`, {
				method: 'PUT',
	...
	async function deleteCharacter(charId) {
		try {
			...
			if (!response.ok) console.error('Failed to delete character', response.status);
		} catch (error) {
	...
	function _getSelectValues(event) {
	...
	function buildProfileSummary(charId) {
		...
		const sp = speciesNameById[charId]
			? resolveLabel(labelMaps.species, speciesNameById[charId], speciesNameById[charId])
			: 'Sin especie';
		return `${bg} / ${sp}`;
	}
...
<section class="character-management" aria-labelledby="manage-character-title">
	<div class="manage-header">
		<h2 id="manage-character-title" class="manage-title">GESTION DE PERSONAJES</h2>
	</div>
								...
								<div class="manage-readonly-item">
									<span class="manage-readonly-label manage-readonly-expand">
										...
										<Button
											...
											aria-expanded={loadoutOpenById[character.id] ? 'true' : 'false'}
										>
											{loadoutOpenById[character.id] ? 'ocultar detalle' : 'ver detalle'}
										</Button>
									</span>
									{#if !loadoutOpenById[character.id]}
										<span class="manage-readonly-value">{buildLoadoutSummary(character.id)}</span>
									{:else}
										<div class="manage-loadout-details" transition:fade={{ duration: 120 }}>
											<div class="manage-loadout-line">
												...
												<span class="manage-loadout-val"
													>{formatSelectionListCompact(skillsById[character.id])}</span
												>
											</div>
											<div class="manage-loadout-line">
												<span class="manage-loadout-key">Herramientas:</span>
												<span class="manage-loadout-val"
													>{formatSelectionListCompact(toolsById[character.id])}</span
												>
											</div>
											<div class="manage-loadout-line">
												<span class="manage-loadout-key">Armadura:</span>
												<span class="manage-loadout-val"
													>{formatSelectionListCompact(armorById[character.id])}</span
												>
											</div>
											<div class="manage-loadout-line">
												<span class="manage-loadout-key">Armas:</span>
												<span class="manage-loadout-val"
													>{formatSelectionListCompact(weaponsById[character.id])}</span
												>
											</div>
											<div class="manage-loadout-line">
												<span class="manage-loadout-key">Items:</span>
												<span class="manage-loadout-val"
													>{formatSelectionListCompact(itemsById[character.id])}</span
												>
											</div>
											<div class="manage-loadout-line">
												...
												<span class="manage-loadout-val"
													>{trinketById[character.id]
														? labelOf.get(trinketById[character.id]) || trinketById[character.id]
														: 'Ninguno'}</span
												>
											</div>
										</div>
					...
					<div class="manage-form">
						<div class="manage-field">
							...
							<Input
								...
								oninput={(event) =>
									(nameById = updateField(nameById, character.id, event.currentTarget.value))}
								maxlength="40"
							/>
						</div>
						<div class="manage-field">
							...
							<Input
								...
								oninput={(event) =>
									(playerById = updateField(playerById, character.id, event.currentTarget.value))}
								maxlength="40"
							/>
						</div>
						<div class="manage-grid-fields">
							<div class="manage-field">
								...
								<Input
									...
									oninput={(event) =>
										(hpMaxById = updateField(hpMaxById, character.id, event.currentTarget.value))}
								/>
							</div>
							<div class="manage-field">
							...
							<div class="manage-grid-two">
								...
								<label class="manage-field">
									...
									<select
										...
										disabled={!classPrimaryById[character.id]}
									>
										{#if !classPrimaryById[character.id]}
											<option value="">Sin opciones</option>
										{:else}
											<option value="">Sin definir</option>
											{#each subclassOptions.filter((o) => !o.class || o.class === classPrimaryById[character.id]) as option (option.key)}
												<option value={option.key}>{option.label}</option>
											{/each}
										{/if}
									</select>
								...
								<div class="manage-field">
									<span class="label-caps"
										>Idiomas {#if languagesById[character.id]?.length > 0}<span
												class="selection-count">{languagesById[character.id].length}</span
											>{/if}</span
									>
									<MultiSelect
										...
										onchange={(v) =>
											(languagesById = updateListField(languagesById, character.id, v))}
										size={Math.max(3, Math.min(6, languageOptions.length || 3))}
									/>
									<SelectionPills keys={languagesById[character.id] || []} {labelOf} />
								</div>
								<div class="manage-field">
									<span class="label-caps"
										>Idiomas raros {#if rareLanguagesById[character.id]?.length > 0}<span
												class="selection-count">{rareLanguagesById[character.id].length}</span
											>{/if}</span
									>
									<MultiSelect
										...
										onchange={(v) =>
											(rareLanguagesById = updateListField(rareLanguagesById, character.id, v))}
										size={Math.max(3, Math.min(6, rareLanguageOptions.length || 3))}
									/>
									<SelectionPills keys={rareLanguagesById[character.id] || []} {labelOf} />
								</div>
							</div>
							<div class="manage-grid-two">
								<div class="manage-field">
									<span class="label-caps"
										>Skills {#if skillsById[character.id]?.length > 0}<span class="selection-count"
									...
									<MultiSelect
										...
										onchange={(v) => (skillsById = updateListField(skillsById, character.id, v))}
										size={Math.max(4, Math.min(8, skillOptions.length || 4))}
									/>
									<SelectionPills keys={skillsById[character.id] || []} {labelOf} />
								</div>
								<div class="manage-field">
									<span class="label-caps"
										>Herramientas {#if toolsById[character.id]?.length > 0}<span
												class="selection-count">{toolsById[character.id].length}</span
											>{/if}</span
									>
									<MultiSelect
										options={toolOptions}
										selected={toolsById[character.id] || []}
										onchange={(v) => (toolsById = updateListField(toolsById, character.id, v))}
										size={Math.max(4, Math.min(8, toolOptions.length || 4))}
									/>
									<SelectionPills keys={toolsById[character.id] || []} {labelOf} />
								</div>
							</div>
							<div class="manage-grid-two">
								<div class="manage-field">
									<span class="label-caps"
										>Armadura {#if armorById[character.id]?.length > 0}<span class="selection-count"
												>{armorById[character.id].length}/{armorOptions.length}</span
											>{/if}</span
									>
									<MultiSelect
										options={armorOptions}
										selected={armorById[character.id] || []}
										onchange={(v) => (armorById = updateListField(armorById, character.id, v))}
										size={Math.max(3, Math.min(6, armorOptions.length || 3))}
									/>
									<SelectionPills keys={armorById[character.id] || []} {labelOf} />
								</div>
								<div class="manage-field">
									<span class="label-caps"
										>Armas {#if weaponsById[character.id]?.length > 0}<span class="selection-count"
												>{weaponsById[character.id].length}/{weaponOptions.length}</span
											>{/if}</span
									>
									<MultiSelect
										options={weaponOptions}
										selected={weaponsById[character.id] || []}
										onchange={(v) => (weaponsById = updateListField(weaponsById, character.id, v))}
										size={Math.max(3, Math.min(6, weaponOptions.length || 3))}
									/>
									<SelectionPills keys={weaponsById[character.id] || []} {labelOf} />
								</div>
							</div>
						</div>
						...
						<div class="manage-section">
							...
							<div class="manage-grid-two">
								<div class="manage-field">
									<span class="label-caps"
										>Items {#if itemsById[character.id]?.length > 0}<span class="selection-count"
									...
									<MultiSelect
										...
										onchange={(v) => (itemsById = updateListField(itemsById, character.id, v))}
										disabled={itemOptions.length === 0}
										size={Math.max(3, Math.min(6, itemOptions.length || 3))}
									/>
									<SelectionPills keys={itemsById[character.id] || []} {labelOf} />
								</div>
								<label class="manage-field">
						...
						<div class="manage-actions">
							...
							<Button
								...
								disabled={isSavingProfileById[character.id]}
							>
								{isSavingProfileById[character.id] ? 'GUARDANDO...' : 'GUARDAR DATOS'}
							</Button>
							{#if profileFeedbackById[character.id]?.text}
								<span class={`manage-feedback ${profileFeedbackById[character.id].type}`}>
									{profileFeedbackById[character.id].text}
								</span>
							{/if}
							<span class="manage-note"> Subir nivel solo ajusta el nivel por ahora. </span>
							<Button
...
>
	<AlertDialog.Content class="card-base delete-confirm-dialog">
		<AlertDialog.Title class="delete-confirm-title">¿Eliminar personaje?</AlertDialog.Title>
		<AlertDialog.Description class="delete-confirm-desc">
			Esta acción no se puede deshacer. El personaje se eliminará de la sesión para todos los
			jugadores.
