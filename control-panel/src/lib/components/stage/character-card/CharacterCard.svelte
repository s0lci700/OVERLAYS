...
<script lang="ts">
	...
	/** Logical collapsed state — drives aria-expanded and button data-state. */
	let isCollapsed = $state(false);

	/** Visual collapsed state — CSS class applied only after animation completes. */
	...
	const hpClass = $derived(
		hpPercent > HP_THRESHOLDS.HEALTHY
			? 'hp--healthy'
			: hpPercent > HP_THRESHOLDS.INJURED
				? 'hp--injured'
				: 'hp--critical'
	);
...
>
	...
	<div class="char-header">
		...
		{/if}
		<!-- Character photo -->
		<img src={photoSrc} alt={character.name} class="char-photo" loading="lazy" />
		...
		<div class="char-header-actions">
			...
			<div class="char-hp-nums">
				<span class="hp-cur" class:is-critical={hpPercent <= 30}>{character.hp_current}</span>
