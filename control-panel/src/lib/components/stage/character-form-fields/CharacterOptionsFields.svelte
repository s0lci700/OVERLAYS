...
<div class="char-options-grid">
	...
	<label class="char-options-field">
		...
		<select class="selector" bind:value={classSubclass} disabled={subclassOptions.length === 0}>
			{#if subclassOptions.length === 0}
				<option value="">Sin opciones</option>
			{:else}
				<option value="">Sin definir</option>
				{#each subclassOptions.filter((o) => !o.class || o.class === classPrimary) as option (option.key)}
					<option value={option.key}>{option.label}</option>
				{/each}
			{/if}
		</select>
	...
	<label class="char-options-field">
		...
		<select class="selector" bind:value={backgroundFeat} disabled={featOptions.length === 0}>
