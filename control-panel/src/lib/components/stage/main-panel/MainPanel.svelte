...
<script lang="ts">
	...
	import { getQueueItems } from '$lib/derived/stage-queue.svelte';
	...
	// Live reveal queue (stage-queue store) — expired items drop off the panel.
...
<main class="main-panel">
	...
	<HudTabBar {activeTab} onChange={(tab) => (activeTab = tab)} queueCount={queueItems.length} />
		...
		<aside class="main-panel__chronicle-sidebar is-desktop-only" bind:this={sidebarNode}>
			...
			>
				<header class="sidebar-header">
					...
					<span class="tab-badge" style="position:static;">{queueItems.length}</span>
				</header>
				...
				>
					...
					<div class="sidebar-resizer__handle">
						<svg viewBox="0 0 20 10" width="20" height="10" fill="currentColor">
							<path
								d="M4 2h2v2H4V2zm4 0h2v2H8V2zm4 0h2v2h-2V2zM4 6h2v2H4V6zm4 0h2v2H8V6zm4 0h2v2h-2V6z"
								opacity="0.5"
							/>
						</svg>
					</div>
			...
			<section
				...
				style="flex: {logCollapsed
					? '0 0 48px'
					: queueCollapsed
						? '1 1 0%'
						: `${100 - queueFlex} 1 0%`}; border-top: none;"
			>
				<header class="sidebar-header">
					<div class="sidebar-header__group">
						<button
