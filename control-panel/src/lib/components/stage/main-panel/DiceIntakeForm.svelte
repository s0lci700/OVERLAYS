...
<script lang="ts">
	...
	import { characters, SERVER_URL, getSocket } from '$lib/services/socket.svelte';
	import { DICE_RESULT_EVENT, type EventPayloadMap } from '$lib/contracts/events';
	...
	import type { AdvantageMode, DiceType, RollOutcome, RollType } from '$lib/contracts';
	...
	let advantageMode = $state<AdvantageMode>('normal');
	let targetValue = $state<number | null>(null);

	const ADVANTAGE_OPTIONS: Array<{ value: AdvantageMode; label: string }> = [
	...
	// Natural 20/1 only counts on a single d20 — multi-dice sums can't crit.
	const isNat20 = $derived(selectedDie === 20 && dice.count === 1 && dice.result === 20);
	const isNat1 = $derived(selectedDie === 20 && dice.count === 1 && dice.result === 1);

	/** Resolved outcome vs target. Undefined when there's no target and no crit (neutral roll). */
	const outcome = $derived.by((): RollOutcome | undefined => {
		if (dice.result === null || total === null) return undefined;
		if (isNat20) return 'critical_success';
		if (isNat1) return 'critical_failure';
		if (targetValue !== null) return total >= targetValue ? 'success' : 'failure';
		return undefined;
	});

	// Initiative has no target number; attacks roll against CA, everything else CD.
	const showTarget = $derived(selectedRollType !== 'initiative_roll');
	const targetLabel = $derived(selectedRollType === 'attack_roll' ? 'CA OBJETIVO' : 'CD OBJETIVO');
	...
	$effect(() => {
		...
		if (!portraitEl || charId === prevCharId) {
			prevCharId = charId;
			return;
		}
		prevCharId = charId;
	...
	const handleTargetWheel = makeWheelHandler(
		() => targetValue ?? 0,
		(v) => (targetValue = v),
		0,
		40,
		(v) => (v === 0 ? (targetValue = null) : (targetValue = v))
	);

	async function handleSubmit() {
		if (dice.result === null || total === null || !selectedCharId || isSubmitting) return;
		isSubmitting = true;
		errorMessage = '';
		try {
			// Socket-first: the server relays DICE_RESULT_EVENT to all surfaces,
			// persists the roll, and mirrors the legacy 'dice_rolled' broadcast.
			getSocket().emit(DICE_RESULT_EVENT, {
				rollType: selectedRollType,
				diceType: `d${selectedDie}` as DiceType,
				rolledDice: dice.result,
				selectedDie: dice.result,
				modifier: dice.modifier,
				advantage: advantageMode,
				total,
				targetValue: showTarget && targetValue !== null ? targetValue : undefined,
				outcome,
				criticalState: outcome === 'critical_success' || outcome === 'critical_failure',
				charId: selectedCharId,
				characterName: selectedChar?.name,
				source: 'stage',
				timestamp: new Date().toISOString()
			} as EventPayloadMap['diceResultEvent']);
...
>
	...
	<div class="dice-form__selector-group">
		...
		<!-- Advantage / disadvantage toggle — metadata for overlay styling, never auto-selects -->
		<div class="dice-form__field">
			<span class="dice-form__label" id="advantage-group-label">CONDICIÓN DE TIRADA</span>
			<div class="die-group" role="radiogroup" aria-labelledby="advantage-group-label">
				{#each ADVANTAGE_OPTIONS as option (option.value)}
					<button
						type="button"
						class="die-btn"
						class:is-active={advantageMode === option.value}
						role="radio"
						aria-checked={advantageMode === option.value}
						onclick={() => (advantageMode = option.value)}
					>
						{option.label}
					</button>
				{/each}
	...
	<div class="dice-form__row">
		...
		{#if showTarget}
			<div onwheel={handleTargetWheel} class="dice-form__field dice-form__field--sm">
				<label
					class="dice-form__label"
					for="dice-target"
					title="Número objetivo a igualar o superar (opcional)">{targetLabel}</label
				>
				<input
					class="dice-form__input"
					id="dice-target"
	...
	{#if shouldRenderTotal}
		<div bind:this={totalEl} class="dice-form__total" title="Resultado final (Suma + Modificador)">
			...
			{#if outcome}
				<span class="dice-form__outcome" data-outcome={outcome}>
					{OUTCOME_LABELS[outcome]}
					{#if targetValue !== null && !isNat20 && !isNat1}
						<span class="dice-form__outcome-target">VS {targetValue}</span>
					{/if}
				</span>
			{/if}
		</div>
