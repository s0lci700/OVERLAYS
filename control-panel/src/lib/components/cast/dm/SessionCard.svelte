<script>
	...
	const classLabel = $derived(
		character.class_name ? `${character.class_name} L${character.level}` : ''
	);
	const photoSrc = $derived(resolvePhotoSrc(character.portrait, SERVER_URL));
</script>
...
>
	...
	<div class="sc-body">
		...
		<div class="sc-hp-nums">
			<span class="sc-hp-cur" class:is-critical={hpPercent <= 30}>{character.hp_current}</span>
			...
			<div class="sc-resources">
				{#each character.resources as resource (resource.id)}
					<div class="sc-resource-row">
						...
						<div
							class="sc-pips"
							aria-label="{resource.name}: {resource.pool_current}/{resource.pool_max}"
						>
