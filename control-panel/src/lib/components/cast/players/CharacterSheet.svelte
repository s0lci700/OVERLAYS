<script lang="ts">
	...
	import { resolvePhotoSrc } from '$lib/services/utils';
	import { getOptimizedSrc, generateSrcset } from '$lib/services/responsive-image';
	...
	const resolvedPortrait = $derived(resolvePhotoSrc(character?.portrait, SERVER_URL));
	const selectedPhoto = $derived(
		resolvedPortrait ? (getOptimizedSrc(resolvedPortrait) ?? resolvedPortrait) : null
	);
	const selectedPhotoSrcset = $derived(resolvedPortrait ? generateSrcset(resolvedPortrait) : null);
</script>
